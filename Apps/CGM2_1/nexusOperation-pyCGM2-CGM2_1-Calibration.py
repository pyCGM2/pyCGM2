...
#            cgm.CGM.reLabelOldOutputs(acqStatic) 
        ...
        inputs = json.loads(open(str(pyCGM2.CONFIG.PYCGM2_APPDATA_PATH+"CGM2_1-pyCGM2.inputs")).read(),object_pairs_hook=OrderedDict)
        ...
        # ----------------------SAVE-------------------------------------------
        if os.path.isfile(DATA_PATH + subject + "-CGM2_1-pyCGM2.model"):
            logging.warning("previous model removed")
            os.remove(DATA_PATH + subject + "-CGM2_1-pyCGM2.model")


        modelFile = open(DATA_PATH + subject+"-CGM2_1-pyCGM2.model", "w")
