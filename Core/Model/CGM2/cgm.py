...
class CGM1ModelInf(CGM):
    ...
    def calibrationProcedure(self):
        ...
        dictRef["Left Foot"]={"TF" : {'sequence':"ZXiY", 'labels':   ["LTOE","LAJC","-","LAJC"]} } # uncorrected Foot - use shank flexion axis (Y) as second axis
        dictRef["Right Foot"]={"TF" : {'sequence':"ZXiY", 'labels':   ["RTOE","RAJC","-","RAJC"]} } # uncorrected Foot - use shank flexion axis (Y) as second axis         
        ...
        dictRefAnatomical["Left Foot"]={'sequence':"ZXiY", 'labels':  ["LTOE","LHEE","-","LAJC"]}    # corrected foot      
        dictRefAnatomical["Right Foot"]={'sequence':"ZXiY", 'labels':  ["RTOE","RHEE","-","RAJC"]}    # corrected foot
    ...
    def calibrate(self,aquiStatic, dictRef, dictAnatomic,  options=None):
        ...
        # check presence of points need for anatomical calibration in btkAcquisition
        for label in dictAnatomic["Pelvis"]['labels']:
            if not btkTools.isPointExist(aquiStatic,label):
                raise Exception("[pycga] Pre-anatomical  calibration checking : Pelvis point %s doesn't exist"% label )

        for label in dictAnatomic["Left Thigh"]['labels']: 
            if not btkTools.isPointExist(aquiStatic,label):
                raise Exception("[pycga] Pre-anatomical  calibration checking : Left Thigh point %s doesn't exist"% label )

        for label in dictAnatomic["Right Thigh"]['labels']: 
            if not btkTools.isPointExist(aquiStatic,label):
                raise Exception("[pycga] Pre-anatomical  calisbration checking : Right Thigh point %s doesn't exist"% label )
            
        for label in dictAnatomic["Left Shank"]['labels']: 
            if not btkTools.isPointExist(aquiStatic,label):
                raise Exception("[pycga] Pre-anatomical  calibration checking : Left Shank point %s doesn't exist"% label )

        for label in dictAnatomic["Right Shank"]['labels']: 
            if not btkTools.isPointExist(aquiStatic,label):
                raise Exception("[pycga] Pre-anatomical  calibration checking : Right Shank point %s doesn't exist"% label )
        ...
        #self.displayStaticAxis(aquiStatic)
...
#    def getTibialRotation(self):
#        ax1 = self.getSegment("Left Thigh").anatomicalFrame.static.m_axisY
#        ax2 = self.getSegment("Left Shank").anatomicalFrame.static.m_axisY
#
#        angle= np.arccos(np.dot(ax2,ax1))
#        
#        print "****** right Tibial Rotation **********"
#        print np.sign(np.cross(ax2,ax1)[2])*angle*360.0/(2.0*np.pi)
#        
#        ax1 = self.getSegment("Right Thigh").anatomicalFrame.static.m_axisY
#        ax2 = self.getSegment("Right Shank").anatomicalFrame.static.m_axisY
#
#        angle= np.arccos(np.dot(ax2,ax1))
#        
#        print "****** right Tibial Rotation **********"
#        print np.sign(np.cross(ax2,ax1)[2])*angle*360.0/(2.0*np.pi)
    ...
    def getFootOffset(self, side = "both",verbose = False):
...
#                pt3= self.getSegment("Left Thigh").static.
    ...
    def getViconFootOffset(self):
        """ 
        .. note: 
        standard vicon : positive = dorsiflexion  and abduction
        """        
        
        spf_l = self.mp_computed["leftStaticPlantarFlexion"] * -1.0
        print " Left staticPlantarFlexion offset"         
        print spf_l
        
        sro_l = self.mp_computed["leftStaticRotOff"] * -1.0
        print " Left staticRotation offset"         
        print sro_l
        
        spf_r = self.mp_computed["rightStaticPlantarFlexion"] * -1.0
        print " Right staticPlantarFlexion offset"        
        print spf_r
        
        sro_r = self.mp_computed["rightStaticRotOff"] 
        print " Right staticRotation offset"         
        print sro_r         
    ...
    def getViconThighOffset(self, side):
        ...
        if side  == "Left":             
            return self.mp_computed["leftThighOffset"] * -1.0
 
        if side  == "Right":             
            return self.mp_computed["rightThighOffset"]        
    ...
    def getViconShankOffset(self, side):
        ...
        if side  == "Left":             
            return self.mp_computed["leftShankOffset"] * -1.0
 
        if side  == "Right":             
            return self.mp_computed["rightShankOffset"]        
    ...
    def displayStaticAxis(self,aquiStatic):

        seg=self.getSegment("Left Thigh")
        val =  np.dot(seg.anatomicalFrame.static.getRotation() , np.array([100.0,0.0,0.0])) + seg.anatomicalFrame.static.getTranslation()
        btkTools.smartAppendPoint(aquiStatic,"LThigh_X",val*np.ones((aquiStatic.GetPointFrameNumber(),3)),desc="")
        val =  np.dot(seg.anatomicalFrame.static.getRotation() , np.array([0.0,100.0,0.0])) + seg.anatomicalFrame.static.getTranslation()
        btkTools.smartAppendPoint(aquiStatic,"LThigh_Y",val*np.ones((aquiStatic.GetPointFrameNumber(),3)),desc="")
        val =  np.dot(seg.anatomicalFrame.static.getRotation() , np.array([0.0,0,100.0])) + seg.anatomicalFrame.static.getTranslation()
        btkTools.smartAppendPoint(aquiStatic,"LThigh_Z",val*np.ones((aquiStatic.GetPointFrameNumber(),3)),desc="")


        seg=self.getSegment("Left Shank")
        val =  np.dot(seg.anatomicalFrame.static.getRotation() , np.array([100.0,0.0,0.0])) + seg.anatomicalFrame.static.getTranslation()
        btkTools.smartAppendPoint(aquiStatic,"LShank_X",val*np.ones((aquiStatic.GetPointFrameNumber(),3)),desc="")
        val =  np.dot(seg.anatomicalFrame.static.getRotation() , np.array([0.0,100.0,0.0])) + seg.anatomicalFrame.static.getTranslation()
        btkTools.smartAppendPoint(aquiStatic,"LShank_Y",val*np.ones((aquiStatic.GetPointFrameNumber(),3)),desc="")
        val =  np.dot(seg.anatomicalFrame.static.getRotation() , np.array([0.0,0,100.0])) + seg.anatomicalFrame.static.getTranslation()
        btkTools.smartAppendPoint(aquiStatic,"LShank_Z",val*np.ones((aquiStatic.GetPointFrameNumber(),3)),desc="")
        
        seg=self.getSegment("Right Thigh")
        val =  np.dot(seg.anatomicalFrame.static.getRotation() , np.array([100.0,0.0,0.0])) + seg.anatomicalFrame.static.getTranslation()
        btkTools.smartAppendPoint(aquiStatic,"RThigh_X",val*np.ones((aquiStatic.GetPointFrameNumber(),3)),desc="")
        val =  np.dot(seg.anatomicalFrame.static.getRotation() , np.array([0.0,100.0,0.0])) + seg.anatomicalFrame.static.getTranslation()
        btkTools.smartAppendPoint(aquiStatic,"RThigh_Y",val*np.ones((aquiStatic.GetPointFrameNumber(),3)),desc="")
        val =  np.dot(seg.anatomicalFrame.static.getRotation() , np.array([0.0,0,100.0])) + seg.anatomicalFrame.static.getTranslation()
        btkTools.smartAppendPoint(aquiStatic,"RThigh_Z",val*np.ones((aquiStatic.GetPointFrameNumber(),3)),desc="")


        seg=self.getSegment("Right Shank")
        val =  np.dot(seg.anatomicalFrame.static.getRotation() , np.array([100.0,0.0,0.0])) + seg.anatomicalFrame.static.getTranslation()
        btkTools.smartAppendPoint(aquiStatic,"RShank_X",val*np.ones((aquiStatic.GetPointFrameNumber(),3)),desc="")
        val =  np.dot(seg.anatomicalFrame.static.getRotation() , np.array([0.0,100.0,0.0])) + seg.anatomicalFrame.static.getTranslation()
        btkTools.smartAppendPoint(aquiStatic,"RShank_Y",val*np.ones((aquiStatic.GetPointFrameNumber(),3)),desc="")
        val =  np.dot(seg.anatomicalFrame.static.getRotation() , np.array([0.0,0,100.0])) + seg.anatomicalFrame.static.getTranslation()
        btkTools.smartAppendPoint(aquiStatic,"RShank_Z",val*np.ones((aquiStatic.GetPointFrameNumber(),3)),desc="")   
...
#            seg.anatomicalFrame.addMotionFrame(frame)
    ...
    def displayMotionAxis(self,acqui):
        ...
        print "motion Axis"
        print acqui.GetPointFrameNumber()
        print len(seg.anatomicalFrame.motion)
        for i in range(0,acqui.GetPointFrameNumber()):
            valX[i,:]= np.dot(seg.anatomicalFrame.motion[i].getRotation() , np.array([100.0,0.0,0.0])) + seg.anatomicalFrame.motion[i].getTranslation()
            valY[i,:]= np.dot(seg.anatomicalFrame.motion[i].getRotation() , np.array([0.0,100.0,0.0])) + seg.anatomicalFrame.motion[i].getTranslation()
            valZ[i,:]= np.dot(seg.anatomicalFrame.motion[i].getRotation() , np.array([0.0,0.0,100.0])) + seg.anatomicalFrame.motion[i].getTranslation()
            
        btkTools.smartAppendPoint(acqui,"LThigh_X",valX,desc="")
        btkTools.smartAppendPoint(acqui,"LThigh_Y",valY,desc="")
        btkTools.smartAppendPoint(acqui,"LThigh_Z",valZ,desc="")
        ...
        btkTools.smartAppendPoint(acqui,"LShank_X",valX,desc="")
        btkTools.smartAppendPoint(acqui,"LShank_Y",valY,desc="")
        btkTools.smartAppendPoint(acqui,"LShank_Z",valZ,desc="")
        ...
        btkTools.smartAppendPoint(acqui,"RThigh_X",valX,desc="")
        btkTools.smartAppendPoint(acqui,"RThigh_Y",valY,desc="")
        btkTools.smartAppendPoint(acqui,"RThigh_Z",valZ,desc="")
        ...
        btkTools.smartAppendPoint(acqui,"RShank_X",valX,desc="")
        btkTools.smartAppendPoint(acqui,"RShank_Y",valY,desc="")
        btkTools.smartAppendPoint(acqui,"RShank_Z",valZ,desc="")
